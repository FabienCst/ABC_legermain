<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\Actualite $actualite
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Actualite'), ['action' => 'edit', $actualite->idActualite]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Actualite'), ['action' => 'delete', $actualite->idActualite], ['confirm' => __('Are you sure you want to delete # {0}?', $actualite->idActualite)]) ?> </li>
        <li><?= $this->Html->link(__('List Actualites'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Actualite'), ['action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="actualites view large-9 medium-8 columns content">
    <h3><?= h($actualite->idActualite) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Titre') ?></th>
            <td><?= h($actualite->titre) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('IdActualite') ?></th>
            <td><?= $this->Number->format($actualite->idActualite) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Date') ?></th>
            <td><?= h($actualite->date) ?></td>
        </tr>
    </table>
    <div class="row">
        <h4><?= __('Description') ?></h4>
        <?= $this->Text->autoParagraph(h($actualite->description)); ?>
    </div>
</div>
